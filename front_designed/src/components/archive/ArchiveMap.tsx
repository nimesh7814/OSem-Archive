...
const StationsFocusController: React.FC<{
  stations: MapStation[];
  fitStationsToken?: number;
}> = ({ stations, fitStationsToken }) => {
  const map = useMap();
  const lastTokenRef = useRef<number | null>(null);

  useEffect(() => {
    if (fitStationsToken == null || lastTokenRef.current === fitStationsToken)
      return;

    lastTokenRef.current = fitStationsToken;
    if (stations.length === 0) return;

    const positions = stations.map(
      (station) => [station.coordinates[1], station.coordinates[0]] as [number, number],
    );
    map.fitBounds(positions, { padding: [60, 60], maxZoom: 13 });
  }, [stations, fitStationsToken, map]);
  ...
  return (
    ...
    >
      ...
      />
        ...
        <StationsFocusController stations={stations} fitStationsToken={fitStationsToken} />
