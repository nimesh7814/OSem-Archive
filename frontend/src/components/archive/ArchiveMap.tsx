...
import { MapContainer, TileLayer, useMap, useMapEvents, Polygon, CircleMarker, Tooltip } from 'react-leaflet';
import type { Map as LeafletMap } from 'leaflet';
import 'leaflet/dist/leaflet.css';

import { motion, AnimatePresence } from 'framer-motion';
import { X, Info, ExternalLink, Square, Hexagon, Hand, Eraser, Check, Loader2, Plus, Minus } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { Button } from "@/components/ui/button";
import { fetchStationInfo, type MapStation, type StationInfo, type GeoPoint } from "@/lib/stationApi";
...
const toLatLngs = (ring: GeoPoint[]): [number, number][] => ring.map(([lng, lat]) => [lat, lng]);

const MapController: React.FC<{ center: GeoPoint; zoom: number }> = ({ center, zoom }) => {
  const map = useMap();
  const lastRef = useRef<{ c: GeoPoint; z: number } | null>(null);
  useEffect(() => {
    const last = lastRef.current;
    if (!last || last.c[0] !== center[0] || last.c[1] !== center[1] || last.z !== zoom) {
...
const ArchiveMap = ({
  ...
  const activeAOIStroke = aoi?.properties.source === 'rectangle' ? '#3b82f6' : '#10b981';
  const activeAOIFill = aoi?.properties.source === 'rectangle' ? '#3b82f6' : '#10b981';
  ...
  return (
    ...
    >
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-xl border border-slate-200 shadow-lg overflow-hidden">
          <div className="flex flex-col">
            <Button
              ...
              onClick={() => handleToolChange(drawingMode === 'rectangle' ? 'none' : 'rectangle')}
            >
            ...
            <Button
              ...
              onClick={() => handleToolChange(drawingMode === 'polygon' ? 'none' : 'polygon')}
            >
      ...
      <div className="absolute top-6 right-6 z-[600] flex flex-col bg-white/95 rounded-xl border border-slate-200 shadow-lg overflow-hidden">
        <Button variant="ghost" size="icon" onClick={zoomIn} className="h-9 w-9 rounded-none border-b border-slate-200 text-slate-700 hover:bg-slate-100" title="Zoom in">
          <Plus className="w-4 h-4" />
        </Button>
        <Button variant="ghost" size="icon" onClick={zoomOut} className="h-9 w-9 rounded-none text-slate-700 hover:bg-slate-100" title="Zoom out">
        ...
        {isLoadingStations && (
          ...
          >
            <div className="flex items-center gap-3 rounded-2xl bg-white px-5 py-3 shadow-xl border border-slate-200">
              ...
              <span className="text-sm font-bold text-slate-700">Loading stations…</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isSearching && (
          ...
          >
            ...
            <span className="text-xs font-bold text-slate-700">Running query…</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <MapContainer
        ...
        style={{ background: '#dfeaf4', cursor: drawingMode !== 'none' ? 'crosshair' : 'grab' }}
      >
        <TileLayer
        ...
        {stations.map((station) => (
          ...
          >
            <Tooltip direction="top" offset={[0, -6]} opacity={0.95} sticky>
              <span className="text-xs font-bold">{station.name ?? station.id}</span>
            </Tooltip>
          </CircleMarker>
        ))}
      </MapContainer>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Station ID: {selectedStation.id}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 py-4 border-y border-slate-100">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Sensors</p>
                  <p className="text-lg font-black text-slate-900">
                    {stationInfoLoading ? '…' : (selectedStationInfo?.total_sensors ?? selectedStation.sensors ?? '—')}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Readings</p>
                  <p className="text-lg font-black text-slate-900">
                    {stationInfoLoading ? '…' : (selectedStationInfo?.total_readings?.toLocaleString() ?? selectedStation.readings ?? '—')}
                  </p>
                </div>
              </div>

              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Sensor Types</p>
                <div className="flex flex-wrap gap-1.5">
                  {(selectedStationInfo?.categories ? selectedStationInfo.categories.split(',').map((item) => item.trim()).filter(Boolean) : selectedStation.types ?? []).slice(0, 4).map((type) => (
                    <span key={type} className="px-2 py-1 bg-slate-100 rounded-md text-[10px] font-bold text-slate-600">{type}</span>
                  ))}
                  {(((selectedStationInfo?.categories ? selectedStationInfo.categories.split(',').map((item) => item.trim()).filter(Boolean) : selectedStation.types ?? []).length) > 4) && (
