...
const BulkModeView = ({
  ...
  const handleSearch = async () => {
    ...
    const selectedSensors = filters.sensorTypes.length > 0 ? filters.sensorTypes : ['All Sensors'];
    ...
    try {
      ...
      const totalStations = newTableData.reduce((acc, row) => acc + row.stations, 0);
      const totalSensors = newTableData.reduce((acc, row) => acc + row.sensors, 0);
      const totalSizeMB = newTableData.reduce((acc, row) => acc + row.sizeMB, 0);
  ...
  const handleDownload = async (row: RegionRow) => {
    ...
    try {
      ...
      const filename = match?.[1] ?? `${row.country}-${row.region}-country-region-data.zip`;
    ...
    >
      <div className="p-8 max-w-6xl mx-auto w-full">
        <div className="space-y-2 mb-10">
          <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight">Country / Region Export</h2>
          <p className="text-lg text-gray-500 font-medium">Download complete datasets by country or region.</p>
        </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="space-y-3">
                <Label className="ml-1 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Sensors</Label>
                <SensorSelector
              ...
              <div className="space-y-3">
                <Label className="ml-1 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">From Date</Label>
                <Input
                  type="date"
              ...
              <div className="space-y-3">
                <Label className="ml-1 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">To Date</Label>
                <Input
                  type="date"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-end">
              ...
              <div className="flex gap-3">
                ...
                >
                  {localSearching || isSearching ? <Loader2 className="w-5 h-5 animate-spin" /> : <Search className="w-5 h-5" />}
                  Query
                </Button>
              </div>
            </div>
          ...
          <div className="mb-8 rounded-[32px] border border-gray-100 bg-white/80 p-6 shadow-sm">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.24em] text-blue-500">Last Query</p>
                  <p className="mt-1 text-sm font-bold text-gray-700">The table below reflects this query only after pressing Query.</p>
                </div>
                <Badge className="rounded-full bg-blue-50 text-blue-700 hover:bg-blue-50 border border-blue-100 px-3 py-1 font-bold">Saved</Badge>
              </div>

              <div className="flex flex-wrap gap-2">
                <Badge variant="secondary" className="rounded-full bg-blue-100 text-blue-800 border-none px-3 py-1 font-bold">{lastQuery.country}</Badge>
                <Badge variant="secondary" className="rounded-full bg-blue-100 text-blue-800 border-none px-3 py-1 font-bold">{lastQuery.region}</Badge>
                <Badge variant="secondary" className="rounded-full bg-slate-100 text-slate-700 border-none px-3 py-1 font-bold">{lastQuery.fromDate} → {lastQuery.toDate}</Badge>
                {lastQuery.sensors.slice(0, 5).map((sensor) => (
                  <Badge key={sensor} variant="secondary" className="rounded-full bg-emerald-100 text-emerald-800 border-none px-3 py-1 font-bold">
                    {sensor}
                  </Badge>
                ))}
                {lastQuery.sensors.length > 5 && (
                  <Badge variant="secondary" className="rounded-full bg-emerald-200 text-emerald-900 border-none px-3 py-1 font-bold">
                    +{lastQuery.sensors.length - 5}
                  </Badge>
                )}
              </div>
                  ...
                  {tableData.map((row) => {
                    ...
                    return (
                      <tr key={rowKey} className="hover:bg-gray-50/50 transition-colors group">
                        <td className="px-10 py-6 font-black text-gray-900 text-lg tracking-tight">{row.country}</td>
                        <td className="px-10 py-6 text-gray-600 font-bold">{row.region}</td>
                        <td className="px-10 py-6 text-gray-600 font-bold">{row.stations.toLocaleString()}</td>
                        <td className="px-10 py-6 text-gray-600 font-bold">{row.sensors.toLocaleString()}</td>
                        <td className="px-10 py-6 text-gray-600 font-bold">{row.sizeMB.toFixed(2)} MB</td>
                        <td className="px-10 py-6 text-right">
                          ...
                          >
                            {downloadingRow === rowKey ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Download className="w-4 h-4 mr-1" />}
                            {downloadingRow === rowKey ? 'Downloading' : 'Download'}
                          </Button>
                        </td>
                      </tr>
                    );
