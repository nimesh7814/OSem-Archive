...
type TemplateSketcher = {
	generateSketch: (
		box: unknown,
		options?: { encoding?: string | undefined },
	) => string
}
let templateSketcherPromise: Promise<TemplateSketcher> | undefined
...
const getTemplateSketcher = (): Promise<TemplateSketcher> => {
	if (!templateSketcherPromise) {
		templateSketcherPromise = import('@sensebox/sketch-templater').then(
			({ default: SketchTemplater }) =>
				new SketchTemplater(cfg) as TemplateSketcher,
		)
	}
	return templateSketcherPromise
}
...
const handleSketch = async (
	...
	const templateSketcher = await getTemplateSketcher()
